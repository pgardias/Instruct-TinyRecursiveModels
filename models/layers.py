...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            mask = attention_mask.unsqueeze(1).unsqueeze(2).to(query.dtype)
            attn_kwargs["attn_mask"] = (mask - 1.0) * 1e9
        attn_output = scaled_dot_product_attention(query=query, key=key, value=value, is_causal=self.causal, **attn_kwargs)
